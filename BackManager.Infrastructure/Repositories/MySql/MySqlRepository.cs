...
namespace BackManager.Infrastructure
{
        ...
        where TEntity : class, IEntity<TPrimaryKey>, IAggregateRoot<TPrimaryKey>
    {
        ...
        {
            SqlResult sqlResult = SqlVisitorFactory.GetSingleton(SqlType.MySql).WhereSql(lambdaWhere);
        ...
        private static string ToSql(string sql, int pageSize, int pageIndex, string Orderby, bool IsDesc)
        {
            string getOrderBy()
            {
                ...
                if (IsDesc)
                {
                    return $"order by t.{Orderby} DESC";
                }
                else
                {
                    return $"order by t.{Orderby} ";
                }
            }
            ...
            string pageSql = $@"select * from 
                              (
                               {sql}
                                ...
                                where t.{Orderby} > ({pageIndex}-1)*{pageSize}
                                {
                              getOrderBy()
                             } 
                                limit {pageSize}; ";
        ...
        public abstract Task<int> BulkUpdate(List<TEntity> Entities,
